...
TASK_PAGES_PATH = os.path.join(BUILD, 'task-pages.json')
...
def load_task_page_index(path=TASK_PAGES_PATH):
    """Load the manually reviewed exact-task-page map without inferring links."""
    with open(path, encoding='utf-8') as handle:
        index = json.load(handle)
    for field in ('version', 'reviewedAt', 'sourceIndex', 'interpretation', 'tasks'):
        if field not in index:
            raise ValueError(f'task page index missing "{field}"')
    if not isinstance(index['tasks'], dict):
        raise ValueError('task page index tasks must be an object')
    for slug, match in index['tasks'].items():
        if not SLUG.fullmatch(slug):
            raise ValueError(f'task page index has invalid slug "{slug}"')
        if not isinstance(match, dict):
            raise ValueError(f'{slug}: task page mapping must be an object')
        url = match.get('url')
        parsed = urllib.parse.urlsplit(url or '')
        if parsed.scheme not in ('http', 'https') or not parsed.netloc:
            raise ValueError(f'{slug}: task page URL must be absolute HTTP(S)')
        if not isinstance(match.get('matchMethod'), str) or not match['matchMethod']:
            raise ValueError(f'{slug}: task page matchMethod must be a non-empty string')
        ...
        if canonical is not None and (not isinstance(canonical, str) or not SLUG.fullmatch(canonical)):
            raise ValueError(f'{slug}: canonicalTask must be a task slug')
        if match['matchMethod'] == 'legacy-alias' and not canonical:
            raise ValueError(f'{slug}: legacy-alias mapping must name canonicalTask')
    for slug, match in index['tasks'].items():
        canonical = match.get('canonicalTask')
        if not canonical:
            continue
        if canonical == slug or canonical not in index['tasks']:
            raise ValueError(f'{slug}: canonicalTask must name another mapped task')
        canonical_match = index['tasks'][canonical]
        if canonical_match.get('canonicalTask'):
            raise ValueError(f'{slug}: canonicalTask cannot point to another alias')
        if canonical_match['url'] != match['url']:
            raise ValueError(f'{slug}: alias URL must equal canonicalTask URL')
    return index


def attach_task_pages(tasks, index):
    """Attach reviewed exact SOP pages and return honest coverage counts."""
    by_slug = {task['slug']: task for task in tasks}
    unknown = sorted(set(index['tasks']) - set(by_slug))
    if unknown:
        raise ValueError('task page mappings reference unknown tasks: ' + ', '.join(unknown))
    for task in tasks:
        match = index['tasks'].get(task['slug'])
        task['taskPage'] = match['url'] if match else None
        task['taskPageMatch'] = ({
            'matchMethod': match['matchMethod'],
            'canonicalTask': match.get('canonicalTask'),
        } if match else None)
    mapped = [task for task in tasks if task['taskPage']]
    return {
        'tasksWithTaskPage': len(mapped),
        'tasksWithoutTaskPage': len(tasks) - len(mapped),
        # Aliases share the reviewed canonical URL and are one unique page.
        'uniqueTaskPages': len({task['taskPage'] for task in mapped}),
    }
...
def write_library_index(data, out_path):
    ...
    linking separately to a reviewed exact SOP and broader definitive hub. This is the
    ...
    L.append(f"<p>The BlitzMetrics Task Library registers <strong>{st['total']} operational tasks</strong> "
             f"across {st['categories']} categories. Each record opens the current skill; "
             f"{st['tasksWithTaskPage']} have a manually reviewed exact task SOP and "
             f"{st['tasksWithoutTaskPage']} still need one. Broader definitive hubs are labeled separately; "
             f"missing context, access, examples, task pages, or hubs stay visible as gaps. "
    ...
    for c in data['categories']:
        ...
        for t in c['tasks']:
            ...
            task_page = (f' <a href="{html_escape(t["taskPage"])}">Exact task SOP</a>.'
                         if t.get('taskPage') else '')
            article = (f' <a href="{html_escape(t["article"])}">Broader definitive hub</a>.'
                       if t.get('article') else '')
            L.append(f'<li><a href="{task_url}">{label}</a> — {desc}.{html_escape(score)}{task_page}{article}</li>')
        L.append('</ul>')
    L.append('</section>')
...
def main():
    ...
    try:
        task_page_index = load_task_page_index()
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        print(f'ERROR task page index: {exc}')
        sys.exit(1)
    ...
    try:
        task_page_stats = attach_task_pages(all_tasks, task_page_index)
    except ValueError as exc:
        errors.append(str(exc))
        task_page_stats = {
            'tasksWithTaskPage': 0,
            'tasksWithoutTaskPage': len(all_tasks),
            'uniqueTaskPages': 0,
        }
    ...
    data = {'stats': {'total': len(all_tasks),
            ...
            'taskPageIndex': {
                key: task_page_index[key]
                for key in ('version', 'reviewedAt', 'sourceIndex', 'interpretation')
            },
